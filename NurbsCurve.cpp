//
// Created by miaodi on 08/11/2016.
//
...
void NurbsCurve<T, N>::degreeElevate(int t) {
    ...
    int ph = p + t;
    int ph2 = ph / 2;
    Matrix<T, Dynamic, Dynamic> bezalfs = Matrix<T, Dynamic, Dynamic>::Zero(p + t + 1, p +
                                                                                       1); // coefficients for degree elevating the Bezier segment
    ...
    resize(static_cast<int>(c.getControlPoints().rows() * t * 3), ph); // Allocate more control points than necessary
...
template<class T, int N>
void NurbsCurve<T, N>::knotInsertion(T u, int r) {
    NurbsCurve<T, N> c(*this);
    int n = dof;
    int p = c.deg_;
    int m = n + p;
    ...
    Matrix<Matrix<T, N, 1>, Dynamic, 1> R(p + 1);
    for (i = 0; i <= k - p; i++)
        P(i) = c.P(i);
    for (i = k - s; i < n; i++)
        P(i + r) = c.P(i);
    ...
    for (j = 1; j <= r; j++) {
        L = k - p + j;
        for (i = 0; i <= p - j - s; i++) {
            alpha = (u - c.U(L + i)) / (c.U(i + k + 1) - c.U(L + i));
...
template<class T, int N>
void NurbsCurve<T, N>::refineKnotVectorCurve(Matrix<T, Dynamic, 1> X) {
    int n = dof - 1;
    int p = deg_;
    int m = n + p + 1;
    int a, b;
    int r = static_cast<int>(X.rows() - 1);
    NurbsCurve<T, N> c(*this);
...
template
class NurbsCurve<double, 1>;

template
class NurbsCurve<double, 2>;

template
class NurbsCurve<double, 3>;